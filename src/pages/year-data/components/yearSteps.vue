    ...
    <div class="echarts_wrapper" v-show="echartShows">
      ...
      <div class="parameter">
        ...
        <ul>
          <li class="item">
            <span class="value">
              <template v-if="yearSteps!==''">
                <strong>{{yearSteps}}</strong>
                <i>{{$t('components.'+sumUnit)}}</i>
              </template>
              <template v-else>--</template>
            </span>
            <p class="title">{{$t('tag.sports.yearbu')}}</p>
          </li>
          <li class="item">
            <span class="value">
              <template v-if="yearDistance!==''">
                <strong>{{yearDistance}}</strong>
                <i>{{$t('tag.mile')}}</i>
              </template>
              <template v-else>--</template>
            </span>
            <p class="title">{{$t('tag.sports.yearmile')}}</p>
          </li>
          <li class="item">
            <span class="value">
...
export default {
  ...
  methods: {
    getEchartsDataInfo() {
      ...
      this.$axios.getYearsports(postData).then(this.getEchartsDataInfoSucc);
    },
    getEchartsDataInfoSucc(res) {
      ...
      if (res.code === 0 && res.data) {
        ...
        this.yearSteps = getData.outlineData.yearSteps;
        this.yearDistance = getData.outlineData.yearDistance;
        ...
        this.content =
          this.$t("steps.year.a1") +
          this.dailyAveSteps +
          this.$t("steps.year.a2") +
          this.dailyAveCal +
          this.$t("steps.year." + this.statec);
