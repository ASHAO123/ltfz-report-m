<template>
  <div class="report_item_wrapper" ref="alerts">
    ...
    <div class="nodata" v-show="!echartShows">{{$t('tag.blood.noyear')}}</div>
  </div>
</template>
<script>
export default {
  ...
  methods: {
    getEchartsDataInfo () {
      ...
      this.$axios.getYearbloodpressure(postData)
    ...
    getEchartsDataInfoSucc (res) {
      ...
      if (res.code === 0 && res.data) {
        ...
        this.content = this.$t('bloodpre.year.a1')+getData.outlineData.hightc+ this.$t('bloodpre.year.a2') +getData.outlineData.lowc + this.$t('bloodpre.year.'+this.statec)
