<template>
  <div class="report_item_wrapper">
    ...
    <div class="nodata" v-show="!echartShows">{{$t('tag.bloodoxygen.noyear')}}</div>
  </div>
</template>
<script>
export default {
  ...
  methods: {
    getEchartsDataInfo () {
      ...
      this.$axios.getYearbloodoxygen(postData)
    ...
    getEchartsDataInfoSucc (res) {
      ...
      if (res.code === 0 && res.data) {
        ...
        this.content =this.$t('bloodoxygen.year.a1')+this.wholeDayAve.value+this.$t('bloodoxygen.year.'+this.statec)
