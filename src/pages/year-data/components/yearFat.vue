<template>
  <div class="report_item_wrapper">
    ...
    <div class="nodata" v-show="!echartShows">{{$t('tag.fat.noyear')}}</div>
  </div>
</template>
<script>
export default {
  ...
  methods: {
    getEchartsDataInfo () {
      ...
      this.$axios.getYearweary(postData)
    ...
    getEchartsDataInfoSucc (res) {
      ...
      if (res.code === 0 && res.data) {
        ...
        if(this.statec==="text"){
          this.content=this.$t('fat.year.'+this.statec);
        }else if(this.statec==="zhengchang"){
          this.content=this.$t('fat.year.'+this.statec).d1+result+this.$t('fat.year.'+this.statec).d2
        }else {
          this.content=this.$t('fat.year.a1')+result+this.$t('fat.year.'+this.statec)
        }
        // 图表
        this.chart_x_time = []
        this.yearValue = []
        this.chart_x_time = getData.date
        this.yearValue = getData.value
        this.init()
