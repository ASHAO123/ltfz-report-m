...
<template>
  <div class="report_item_wrapper">
    ...
    <div class="nodata" v-show="!echartShows">{{$t('tag.sleep.noyear')}}</div>
  </div>
</template>
<script>
export default {
  ...
  methods: {
    ...
    getEchartsDataInfo () {
      ...
      this.$axios.getYearsleep(postData)
    ...
    getEchartsDataInfoSucc (res) {
      ...
      if (res.code === 0 && res.data) {
        ...
        this.content = this.$t('sleep.year.a1')+this.aveSleepTime_hour+ this.$t("tag.xiaoshi")+this.aveSleepTime_min + this.$t("tag.fen")+ this.$t('sleep.week.a2')+this.aveDeepSleepTime_hour+ this.$t("tag.xiaoshi")+this.aveDeepSleepTime_min+ this.$t("tag.fen")+ this.$t('sleep.year.'+this.statec)
