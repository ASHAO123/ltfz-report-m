<template>
  <div class="report_item_wrapper">
    ...
    <div class="nodata" v-show="!echartShows">{{$t('tag.xinfei.noyear')}}</div>
  </div>
</template>
<script>
export default {
  name: 'yearCardiopulmonary',
  props: {
    sendTime: {
      type: String,
      default: ''
  ...
  methods: {
    getEchartsDataInfo () {
      ...
      this.$axios.getYearheartlung(postData)
    ...
    getEchartsDataInfoSucc (res) {
      ...
      if (res.code === 0 && res.data) {
       ...
       this.statec=getData.statec
        ...
        this.content=this.$t('xinfei.year.'+this.statec)
        }else{
            this.content=this.$t('xinfei.year.'+this.statec).d1+getData.risknum+this.$t('xinfei.year.'+this.statec).d2
        }        
        // 图
        this.chart_x_time = []
        this.heartlungValue = []
