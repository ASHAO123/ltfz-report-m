<template>
  <div class="report_item_wrapper">
    ...
    <div class="nodata" v-show="!echartShows">{{$t('tag.zangfu.noweek')}}</div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    getEchartsDataInfo () {
      ...
      this.$axios.getWeekviscera(postData)
    ...
    getEchartsDataInfoSucc (res) {
      ...
      if (res.code === 0 && res.data) {
        ...
        this.content = this.$t('zangfu.week.'+this.statec)
    ...
    clickWarnData (index) {
      ...
      console.log(this.riskData[index])
    }
  },
  components: {
    'v-warnData': warnData
