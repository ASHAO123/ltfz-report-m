<template>
  <div class="report_item_wrapper" ref="alerts">
    ...
    <!-- 暂无数据 -->
    <div class="nodata" v-if="!echartShows">{{$t('tag.xinlv.noweek')}}</div>
    <!-- 弹窗 -->
  ...
  methods: {
    getEchartsDataInfo () {
      ...
      if (this.sendTime === '') {
        ...
        this.$axios.getWeekheartrate(postData)
    ...
    getEchartsDataInfoSucc (res) {
      ...
      if (res.code === 0 && res.data) {
        ...
        this.content = this.$t('heartrate.week.a1')+this.wholeDayAve.value+ this.$t('heartrate.week.'+this.statec) + this.warnings + this.$t('tag.xinlv.ci') 
