<template>
  <div class="report_item_wrapper">
    ...
    <div class="nodata" v-show="!echartShows">{{$t('tag.xinfei.noweek')}}</div>
  </div>
</template>
<script>
export default {
  name: 'weekCardiopulmonary',
  props: {
    sendTime: {
      type: String,
      default: ''
  ...
  methods: {
    getEchartsDataInfo () {
      ...
      this.$axios.getWeekheartlung(postData)
    ...
    getEchartsDataInfoSucc (res) {
      ...
      if (res.code === 0 && res.data) {
        ...
        this.content=this.$t('xinfei.week.'+this.statec)
        }else{
            this.content=this.$t('xinfei.week.'+this.statec).d1+getData.risknum+this.$t('xinfei.week.'+this.statec).d2
        }
