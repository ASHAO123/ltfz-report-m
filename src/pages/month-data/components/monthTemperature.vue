<template>
  <div class="report_item_wrapper">
    ...
    <div class="nodata" v-show="!echartShows">{{$t('tag.temperature.nomonth')}}</div>
  </div>
</template>
<script>
export default {
  ...
  methods: {
    getEchartsDataInfo () {
      ...
      this.$axios.getMonthbodytempera(postData)
    ...
    getEchartsDataInfoSucc (res) {
      ...
      if (res.code === 0 && res.data) {
        ...
        this.content = this.$t('temperature.month.a1')+this.wholeDayAve.value+this.$t('temperature.month.'+this.statec)
