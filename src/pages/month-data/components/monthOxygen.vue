<template>
  <div class="report_item_wrapper">
    ...
    <div class="nodata" v-show="!echartShows">{{$t('tag.bloodoxygen.nomonth')}}</div>
  </div>
</template>
<script>
export default {
  ...
  methods: {
    getEchartsDataInfo () {
      ...
      this.$axios.getMonthbloodoxygen(postData)
    ...
    getEchartsDataInfoSucc (res) {
      ...
      if (res.code === 0 && res.data) {
        ...
        this.content =this.$t('bloodoxygen.month.a1')+this.wholeDayAve.value+this.$t('bloodoxygen.month.'+this.statec)
