<template>
  <div class="report_item_wrapper">
    ...
    <div class="nodata" v-show="!echartShows">{{$t('tag.bloodsugar.nomonth')}}</div>
  </div>
</template>
<script>
export default {
  ...
  methods: {
    ...
    getEchartsDataInfoSucc (res) {
      ...
      if (res.code === 0 && res.data) {
        ...
        this.chart_x_time = []
        this.bsugarValue = []
        this.bsugarValue = getData.echarts
        ...
        this.content = this.$t('bloodsugar.month.a1')+getData.ave.value+ this.$t('bloodsugar.month.'+this.statec)
        // console.log('sugar=>' + this.chart_x_time + '<br>' + 'this.bsugarValue:=>' + this.bsugarValue)
