<template>
  <div class="report_item_wrapper">
    ...
    <div class="nodata" v-show="!echartShows">{{$t('tag.fat.nomonth')}}</div>
  </div>
</template>
<script>
export default {
  ...
  methods: {
    getEchartsDataInfo () {
      ...
      this.$axios.getMonthweary(postData)
    ...
    getEchartsDataInfoSucc (res) {
      ...
      if (res.code === 0 && res.data) {
        ...
        if(this.statec==="text"){
          this.content=this.$t('fat.month.'+this.statec);
        }else if(this.statec==="zhengchang"){
          this.content=this.$t('fat.month.'+this.statec).d1+result+this.$t('fat.month.'+this.statec).d2
        }else {
          this.content=this.$t('fat.month.a1')+result+this.$t('fat.month.'+this.statec)
        }
        // 图表
        this.chart_x_time = []
        this.monthValue = []
        this.chart_x_time = getData.date
        this.monthValue = getData.value
        this.init()
