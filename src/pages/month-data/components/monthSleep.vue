...
<template>
  <div class="report_item_wrapper">
    ...
    <div class="nodata" v-show="!echartShows">{{$t('tag.sleep.nomonth')}}</div>
  </div>
</template>
<script>
export default {
  ...
  methods: {
    ...
    getEchartsDataInfo () {
      ...
      this.$axios.getMonthsleep(postData)
    ...
    getEchartsDataInfoSucc (res) {
      ...
      if (res.code === 0 && res.data) {
        ...
        this.sleepcolor = getData.outlineData.color

        this.statec=getData.outlineData.statec
        this.content = this.$t('sleep.month.a1')+this.aveSleepTime_hour+ this.$t("tag.xiaoshi")+this.aveSleepTime_min + this.$t("tag.fen")+ this.$t('sleep.week.a2')+this.aveDeepSleepTime_hour+ this.$t("tag.xiaoshi")+this.aveDeepSleepTime_min+ this.$t("tag.fen")+ this.$t('sleep.month.'+this.statec)
        // 图表
