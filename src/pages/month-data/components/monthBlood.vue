<template>
  <div class="report_item_wrapper" ref="alerts">
    ...
    <div class="nodata" v-show="!echartShows">{{$t('tag.blood.nomonth')}}</div>
  </div>
</template>
<script>
export default {
  ...
  methods: {
    getEchartsDataInfo () {
      ...
      this.$axios.getMonthbloodpressure(postData)
    ...
    getEchartsDataInfoSucc (res) {
      ...
      if (res.code === 0 && res.data) {
        ...
        this.content = this.$t('bloodpre.month.a1')+getData.outlineData.hightc+ this.$t('bloodpre.month.a2') +getData.outlineData.lowc + this.$t('bloodpre.month.'+this.statec)
