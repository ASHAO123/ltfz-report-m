<template>
  <div class="report_item_wrapper">
    ...
    <div class="echarts_wrapper" v-show="echartShows">
      ...
      <div class="parameter">
        ...
        <ul>
          <li class="item">
            <span class="value">
              <template v-if="monthSteps!==''">
                <strong>{{monthSteps}}</strong>
                <i>{{$t('components.'+sumUnit)}}</i>
              </template>
              <template v-else>--</template>
            </span>
            <p class="title">{{$t('tag.sports.monthbu')}}</p>
          </li>
          <li class="item">
            <span class="value">
              <template v-if="monthDistance!==''">
                <strong>{{monthDistance}}</strong>
                <i>{{$t('tag.mile')}}</i>
              </template>
              <template v-else>--</template>
            </span>
            <p class="title">{{$t('tag.sports.monthmile')}}</p>
          </li>
          <li class="item">
            <span class="value">
    ...
    <!-- 暂无数据 -->
    <div class="nodata" v-show="!echartShows">{{$t('tag.sports.noyear')}}</div>
  </div>
</template>
<script>
export default {
  ...
  methods: {
    getEchartsDataInfo() {
      ...
      this.$axios.getMonthsports(postData).then(this.getEchartsDataInfoSucc);
    },
    getEchartsDataInfoSucc(res) {
      ...
      if (res.code === 0 && res.data) {
        ...
        this.monthSteps = getData.outlineData.monthSteps;
        this.monthDistance = getData.outlineData.monthDistance;
        ...
        this.content =
          this.$t("steps.month.a1") +
          this.dailyAveSteps +
          this.$t("steps.month.a2") +
          this.dailyAveCal +
          this.$t("steps.month." + this.statec);
