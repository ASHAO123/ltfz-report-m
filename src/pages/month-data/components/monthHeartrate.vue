<template>
  <div class="report_item_wrapper" ref="alerts">
    ...
    <div class="echarts_wrapper" v-show="echartShows">
        ...
        <div class="parameter">
          ...
          <ul>
            ...
            <li class="item">
              <span class="value">
                <template v-if="monthWarnDays!==''">
                  <strong>{{monthWarnDays}}</strong>
                  <i>天</i>
               </template>
               <template v-else>
                 --
              ...
              <p class="title">{{$t('tag.xinlv.yuebaojing')}}</p>
            </li>
          </ul>
        </div>
        <!-- 健康等级评估 -->
    ...
    <!-- 暂无数据 -->
    <div class="nodata" v-show="!echartShows">{{$t('tag.xinlv.nomonth')}}</div>
  </div>
</template>
<script>
export default {
  ...
  methods: {
    getEchartsDataInfo () {
      ...
      this.$axios.getMonthheartrate(postData)
    ...
    getEchartsDataInfoSucc (res) {
      ...
      if (res.code === 0 && res.data) {
        ...
        this.monthWarnDays = getData.outlineData.monthWarnDays

        this.statec=getData.outlineData.statec
        this.content = this.$t('heartrate.month.a1')+this.wholeDayAve.value+ this.$t('heartrate.month.'+this.statec) + this.warnings + this.$t('tag.xinlv.ci') 
