...
<template>
  <article class="cur_con_wrapper">
    <div class="cur_con">
      <header class="header">
        <i class="iconfont">&#xe775;</i>
        <h2>{{$t('title.sleep')}}<span v-show="showData">({{ startTime }} -- {{ endTime }})</span></h2>
        <div class="level" v-show="showData">
          <h3>{{$t('title.zhiliang')}}</h3>
      ...
      <div v-show="showData">
        ...
        <div class="read">
          <header>
            <i class="iconfont">&#xe61d;</i>
            <h2>{{$t('title.secondaryTitle.canshujiedu')}}</h2>
          </header>
          <div class="content">
            <h3></h3>
      ...
      <p v-show="!showData" class="nodata">{{$t('tag.sleep.noday')}}</p>
    </div>
  </article>
</template>
<script>
export default {
  ...
  methods: {
    ...
    getDayDataInfo () {
      ...
      this.$axios.getDaysleep(postData).then(this.getDayDataInfoSucc)
    },
    getDayDataInfoSucc (res) {
      if (res.code === 0 && res.data) {
        ...
        let p
        this.content = ''
        for (let item of this.$t('sleep.day.'+this.statec)) {
          p = '<p>' + item + '</p>'
          this.content += p
        }
        this.init()
      ...
      // this.emit.slFlag = this.showData
      this.$emit('daysleepcode', this.emit)
    },
    init () {
      let _this = this
